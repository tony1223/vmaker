...
class MY_Model extends CI_Model
{
    ...
    /**
     * call overwrite for cache method...
     * (cache_ID,time,(origin_args))
     */
    public function __call($func, $args= Array()){

        $cached = isset($_GET["cached"])? false : true;

        $prefix = "cache__";
        $prefix_length = 7;

        $flush = false;
        if(startsWith($func,$prefix."_")){ // debug
            $flush = true;
        ...
        if(startsWith($func,$prefix)){

            $ci = &get_instance();
            $ci->load->driver('cache');
            $CACHE_ID = $args[0];

            if ($ci->cache->file->is_supported() && !$flush ){
                $data = $ci->cache->file->get($CACHE_ID);
                if($data != false){
                    return $data;
                }
            }
            ...
            $results =  call_user_func_array(array($this, substr($func,$prefix_length)), array_slice($args,2));
            //if it's support but it's closed by other status, we still flush the cache if possible.
            if ($ci->cache->file->is_supported() ){
                @$ci->cache->file->delete($CACHE_ID);
                $ci->cache->file->save($CACHE_ID, $results,  $args[1]);
            }
            return $results;
